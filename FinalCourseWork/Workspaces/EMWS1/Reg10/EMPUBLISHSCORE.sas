...
*** Check LOG_AFFL for missing values ;
if missing( LOG_AFFL ) then do;
   substr(_warn_,1,1) = 'M';
   _DM_BAD = 1;
end;

*** Check LOG_AGE for missing values ;
if missing( LOG_AGE ) then do;
   substr(_warn_,1,1) = 'M';
   _DM_BAD = 1;
end;
...
***  Effect: GENDER ;
_TEMP = 1;
_LP0 = _LP0 + (    1.08328066020289) * _TEMP * _2_0;
_LP0 = _LP0 + (    0.05893603357095) * _TEMP * _2_1;

***  Effect: LOG_AFFL ;
_TEMP = LOG_AFFL ;
_LP0 = _LP0 + (    2.59737501894846 * _TEMP);

***  Effect: LOG_AGE ;
_TEMP = LOG_AGE ;
_LP0 = _LP0 + (   -2.68259151257416 * _TEMP);

***  Effect: REGION ;
_TEMP = 1;
_LP0 = _LP0 + (   -0.00542651737373) * _TEMP * _5_0;
_LP0 = _LP0 + (   -0.33253578959896) * _TEMP * _5_1;
_LP0 = _LP0 + (   -0.00088963645814) * _TEMP * _5_2;
_LP0 = _LP0 + (   -0.09055756082686) * _TEMP * _5_3;

*** Naive Posterior Probabilities;
drop _MAXP _IY _P0 _P1;
_TEMP =     2.92013612242371 + _LP0;
if (_TEMP < 0) then do;
   _TEMP = exp(_TEMP);
   _P0 = _TEMP / (1 + _TEMP);
end;
...
I_ORGYN = REG10DRF[_IY];
U_ORGYN = REG10DRU[_IY];
